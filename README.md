# FRAME 사진동아리 회장 활동 기록

숭실대학교 중앙 사진동아리 **FRAME**의 2026학년도 2학기 회장 활동을 기록하는 Experience Portfolio 저장소입니다.

- **Period:** 2026.07.01–Present
- **Status:** In Progress
- **Role:** President
- **Organization:** FRAME · Soongsil University Central Club · Creative Exhibition Division
- **Official Instagram:** [@ssu_frame](https://www.instagram.com/ssu_frame/?hl=ko)

## Overview

FRAME은 2017년에 창립되어 사진 촬영, 디지털 사진 편집, 정기출사, 소모임 출사 및 전시 활동을 중심으로 활발히 운영되는 디지털 사진동아리입니다.

이 저장소는 회장직 인수인계 이후 진행한 운영체계 점검, 총회, 회칙 개정 및 주요 활동의 배경과 결과를 시간순으로 기록하기 위해 제작했습니다. 공개 웹페이지에서는 동아리 소개와 타임라인을 간결하게 보여주고, 각 타임라인 항목을 선택하면 해당 활동의 상세 내용을 확인할 수 있도록 구성합니다.

## Summary

An ongoing experience portfolio documenting organizational governance, bylaw reform, and major activities as President of FRAME, Soongsil University’s central photography club.

## Timeline

| Date | Activity | Summary | Detail |
|---|---|---|---|
| 2026.07.01 | 회장직 인수인계 및 운영체계 점검 | 전임 운영 자료를 인계받고 회칙·조직·재정·활동 기록 체계를 검토 | [상세 기록](timeline/2026-07-01-handover-and-operational-review.md) |
| 2026.07.10 | 회칙 개정 임시총회 | 2학기 운영체계 개편을 위한 온라인 임시총회 진행 및 제3차 개정안 의결 | [상세 기록](timeline/2026-07-10-extraordinary-general-meeting.md) |

## 2026.07.10 Bylaw Reform

기존 회칙과 실제 운영 방식 사이의 불일치를 해소하고, 역할·참여·의사결정·재정 관리 기준이 명확한 운영체계를 구축하기 위해 제3차 개정을 추진했습니다.

주요 개정 범위는 다음과 같습니다.

- 임원진과 집행부의 역할 및 권한 구분
- 학기별 규모에 대응할 수 있는 집행부 구성 체계
- 총회 사전 공지, 실시간 투표, 의결 및 결과 공개 절차
- 집행부 업무 참여를 위한 책임보증금 제도
- 우수 집행부 및 일반 부원 보상 근거
- 회비·책임보증금·지원금·행사 참가비 등 수입 관리 기준
- 회계 증빙, 결산보고 및 회원 열람 절차
- 정기출사, 전시 및 주요 활동의 공식 반영
- 회원 보호를 위한 제명 사유와 소명 절차

상정된 모든 개정 안건은 2026년 7월 10일 임시총회에서 가결되었으며, 결과를 반영한 제3차 개정 회칙을 작성·배포했습니다.

## Public Documents

- [2026년도 창작전시분과 FRAME 동아리회칙 · 열람용 웹 문서](documents/frame-bylaws-public-2026-07-10.html)
- [회장직 인수인계 및 운영체계 점검](timeline/2026-07-01-handover-and-operational-review.md)
- [회칙 개정 임시총회 상세 기록](timeline/2026-07-10-extraordinary-general-meeting.md)

공개 문서에는 회원 이름, 학번, 연락처, 계좌 정보, 투표 참여자 정보 및 개인별 평가 자료를 포함하지 않습니다.

## Repository Structure

```text
FRAME-Photo-Club-President-Experience/
├─ README.md
├─ index.html
├─ assets/
│  ├─ css/
│  │  ├─ style.css
│  │  ├─ frame-overrides.css
│  │  └─ document.css
│  ├─ js/
│  │  └─ timeline.js
│  └─ images/
│     └─ frame-logo.svg
├─ documents/
│  └─ frame-bylaws-public-2026-07-10.html
├─ timeline/
│  ├─ README.md
│  ├─ 2026-07-01-handover-and-operational-review.md
│  └─ 2026-07-10-extraordinary-general-meeting.md
└─ .github/
   └─ workflows/
      └─ pages.yml
```

## Timeline Update Rule

각 활동은 아래 기준으로 기록합니다.

1. 날짜와 활동명을 타임라인에 추가합니다.
2. 한 줄 요약에는 활동의 핵심 목적이나 결과만 작성합니다.
3. 상세 기록에는 활동 개요, 배경 또는 기획 목적, 진행 과정, 주요 내용, 결과 및 공개 자료를 작성합니다.
4. 별도 배너가 있으면 해당 이미지를 지정합니다.
5. 별도 배너가 없으면 FRAME 기본 로고를 사용합니다.
6. 개인정보와 내부 민감정보는 공개하지 않습니다.

## Banner Policy

```text
타임라인 전용 이미지가 있는 경우
→ 해당 이미지를 배너로 사용

전용 이미지가 없는 경우
→ 메인 포트폴리오의 assets/images/experience/frame-logo.jpg를 기본 배너로 사용
```

## Links

- [Published Experience Page](https://jujushmaterial.github.io/FRAME-Photo-Club-President-Experience/)
- [Main Portfolio](https://jujushmaterial.github.io/)
- [Experience Page](https://jujushmaterial.github.io/experience.html)
- [FRAME Instagram](https://www.instagram.com/ssu_frame/?hl=ko)